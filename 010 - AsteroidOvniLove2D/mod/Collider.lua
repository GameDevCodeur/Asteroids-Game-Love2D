...
function ColliderPlaneOvni(dt)
	...
	if Collider(Game.assetManager:getObject("ovni"), 
				Game.assetManager:getObject("plane"), 
				25 * Game.assetManager:getObject("plane").transform.scale.x) then
				
		-- Décrémente une vie
		if Game.assetManager:getObject("ovni").visible then
			--
			Game.assetManager:getObject("ovni").visible = false
...
function ColliderBulletsOvni(dt)
	...
	for oj = #Game.assetManager:getObject("bullet").weapon.bullets, 1, -1 do	-- Bullets
		...
		if Collider(Game.assetManager:getObject("ovni"), 
					...
					25 * Game.assetManager:getObject("ovni").transform.scale.x) then	
...
function ColliderAstresOvni(dt)
	...
	for oi = #Game.assetManager:getObject("asteroid").astres, 1, -1 do -- Asteroids
		...
		if 	Collider(	Game.assetManager:getObject("asteroid").astres[oi], 
						Game.assetManager:getObject("ovni"), 
						25 * Game.assetManager:getObject("asteroid").astres[oi].transform.scale.x) and 
						Game.assetManager:getObject("ovni").collider == false then	
			...
			-- Explosion
			Game.assetManager:getObject("explode"):start(	
							Game.assetManager:getObject("ovni").transform.translate.x - 80, 
